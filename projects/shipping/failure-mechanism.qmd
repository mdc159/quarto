...
Transport envelope and key system parameters. The rigid system volume $V_{fixed}$ is the single most important unknown in the analysis and is the subject of @sec-critical-unknowns.
:::
...
Representative ideal-vent cases from the MATLAB quasi-static model. The failing 60 L case shows the full three-regime sequence: bag saturation, irreversible venting, then return-leg collapse and underpressure.
:::
...
::: {#fnd-sh-003}
**FND-SH-003:** The rigid connected volume $V_{fixed}$ remains the decisive missing parameter for fully dispositioning the **as-built vented** concept against @eq-no-vent and @eq-return-threshold.

*Disposition: open until measured (@sec-critical-unknowns, @sec-closure).*
:::
...
The single most important unknown is the rigid connected nitrogen volume $V_{fixed}$. It determines where production hardware lies on the screening map in @tbl-thresholds and @fig-min-dp-sweep relative to the no-vent and return-margin limits.

**Why it matters:** Until $V_{fixed}$ is known, the project cannot rigorously classify the present vented architecture as clearly inside, near, or beyond the ideal-vent thresholds in @tbl-thresholds.

**What closes it:** A direct volume measurement (CAD roll-up, gas-fill/water displacement, or equivalent traceable method) recorded against the same manifold boundary used in this analysis.

# Remaining Data To Close The Argument {#sec-closure}

| Item | Owner | What it proves | Status |
|:-----|:------|:---------------|:-------|
| Measure $V_{fixed}$ | TBD | Places the as-built vented hardware on the screening map; enables interpretation against @eq-no-vent and @eq-return-threshold. | Not started |
...
| Characterize actual outward vent threshold (vs ideal vent) | TBD | If the hardware behaves closer to a finite cracking pressure (e.g. 2 psig), thresholds shift (@tbl-thresholds); disposition must use measured or specified behavior. | Not started |
...
::: callout-important
## How to interpret the measured rigid volume

- If $V_{fixed} \le 15.28$ L, the present concept avoids venting under the ideal-vent screening case.
- If $15.28 < V_{fixed} \le 52.55$ L, the present concept can vent and still remain nonnegative on return under ideal venting, but with limited or zero margin.
- If $V_{fixed} > 52.55$ L, ideal outward venting still produces sub-atmospheric return conditions.
- If the bag valve behaves more like a `2 psig` outward vent, the same mechanism remains present but shifts to a larger threshold volume of 109.19 L (@tbl-thresholds).
:::
...
Measurement of $V_{fixed}$ remains mandatory to disposition the **legacy vented** configuration and to confirm that screening assumptions match the built article; it does not negate the overpressure-based argument for vent removal under @sec-review-assumptions.
...
3. For the 22 L bag and stated envelope, ideal-vent screening limits are 15.28 L (no outbound venting) and 52.55 L (nonnegative return); **FND-SH-003** applies until $V_{fixed}$ is measured.
...
6. **Bag-vent removal is a proposed design recommendation**, not an executed hardware change, as stated in @sec-decision-status; formal closure requires @sec-closure.